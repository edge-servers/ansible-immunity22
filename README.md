...
guide you towards a fully working basic immunity22 installation.
...
* for **"production server"** we mean a server (**not a laptop or a desktop computer!**) with public
  ipv4 / ipv6 which is used to host immunity22
...
Choose a working directory **on your local machine** where to put the configuration of immunity22.

This will be useful when you will need to upgrade immunity22.
...
Substitute `immunity22.mydomain.com` with your **production server**'s hostname - **DO NOT REPLACE
`immunity22.mydomain.com` WITH AN IP ADDRESS**, otherwise email sending through postfix will break,
...
Now is time to **deploy immunity22 to the production server**.
...
Substitute `immunity22.mydomain.com` with your production server's hostname.
...
If you want to contribute to `ansible-immunity22` you should run tests
...
look for the word "immunity22_django_cors" in the
[Role variables](#role-variables) section of this document.
...
The ansible-immunity22 only provides abstraction (variables)
for handful of settings available in [django-cors-headers](https://github.com/adamchainz/django-cors-headers)
module. Use the `immunity22_extra_django_settings_instructions` or
...
installing your own trusted certificate and set the `immunity22_ssl_cert` and `immunity22_ssl_key`
...
The first thing you have to do is to setup a valid domain for your immunity22 instance, this means
...
You must be able to add a DNS record for `immunity22.yourdomain.com`, you cannot use an ip address
in place of `immunity22.yourdomain.com`.
